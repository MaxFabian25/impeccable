...
The `typography.md` reference in this project incorporates tactical additions merged from ehmo's `typecraft-guide-skill`: dark-mode weight/tracking compensation, paragraph rhythm, font loading tradeoffs, preload guidance, variable font guidance, bounded `clamp()` sizing, responsive measure coupling, `text-wrap: balance` / `pretty`, `font-optical-sizing: auto`, and all-caps tracking guidance.
