...
`$impeccable` is the foundation. It teaches Codex how to design, period. Every other command in this pack leans on it for design principles, anti-patterns, typography, color, and layout guidance.
...
Most AI-generated UIs fail the same way: generic fonts, purple gradients, card grids on card grids, glassmorphism everywhere. `$impeccable` gives Codex a strong point of view. It loads an opinionated design handbook plus a long list of anti-patterns, then pushes the model to commit to a specific aesthetic direction before writing a single line of code.
