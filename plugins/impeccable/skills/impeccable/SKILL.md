---
name: impeccable
description: Create distinctive, production-grade frontend interfaces with high design quality. Generates creative, polished code that avoids generic AI aesthetics. Use when the user asks to build web components, pages, artifacts, posters, or applications, or when any design skill requires project context. Call with 'craft' for shape-then-build, 'teach' for product/design context setup, 'document' to generate DESIGN.md, 'extract' to pull reusable components and tokens into the design system, 'live' for browser-based variant iteration, or 'pin'/'unpin' to manage local shortcuts.
version: 2.1.7
argument-hint: "[craft|shape · audit|critique · animate|bolder|colorize|delight|layout|overdrive|quieter|typeset · adapt|clarify|distill · harden|onboard|optimize|polish · teach|document|extract|live|pin|unpin] [target]"
...
| Product | PRODUCT.md exists and is not empty or placeholder-only (`[TODO]` markers, under 200 characters). | Run `$impeccable teach`, refresh context, then resume. Never synthesize PRODUCT.md from the user's original prompt alone. |
...
| Craft | `$impeccable craft` has a user-confirmed shape brief for this task. Teach answers and PRODUCT.md never count as shape. | Run `$shape` and wait for explicit brief confirmation. |
| Image | Required visual probes / mocks are generated or skipped with a reason. | Resolve the image-generation gate in `$shape` or [reference/craft.md](reference/craft.md) before code. |
...
- If `hasProduct` is false, empty, or placeholder-only, tell the user: "I need PRODUCT.md before I can do this well. Running `$impeccable teach` now; I'll resume [original task] after." Then run `$impeccable teach`, reload context, and resume the original task. If the original task was `$impeccable craft`, resume into `$shape` before any implementation work.
...
DO NOT use overused fonts like Inter, Roboto, Arial, Open Sans, or system defaults. Do not simply switch to your second-favorite. Every font in the reflex_fonts_to_reject list above is banned. Look further.
...
If this skill is invoked with the argument "live" (e.g., `$impeccable live`), follow the [live variant flow](reference/live.md). This mode starts the local helper, injects the browser picker into the configured page file, then uses `live-poll.mjs` as the foreground event loop while Codex writes and accepts source-backed variants.
