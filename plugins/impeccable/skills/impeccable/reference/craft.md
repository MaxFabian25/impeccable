...
1. Project design context is valid and current from current instructions or project-root `AGENTS.md`.
...
`$impeccable teach` answers and project context do not satisfy the shape gate. They are project-level context only. A compact self-authored brief does not satisfy the shape gate either. Shape is passed only after a separate user response approving the shape brief or an already-confirmed brief supplied by the user.
...
2. Approved visual direction, from generated probes or mocks when image generation is available.
...
If this craft run resumed after `$impeccable teach` created project design context, run shape now. Do not treat the teach interview, `AGENTS.md`, or a summary of project context as a substitute for shape. Shape is task-specific and must cover scope, content/states, visual direction, constraints, anti-goals, probes when applicable, and explicit brief confirmation.
...
Open the result in a browser. In Codex, use Browser or equivalent browser automation when available; otherwise use Playwright or ask the user for screenshots. Inspect screenshots, not just DOM or terminal output.
