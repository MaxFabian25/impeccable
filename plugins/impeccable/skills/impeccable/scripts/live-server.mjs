...
function createRequestHandler({ detectScript, sessionPath, livePath }) {
  return (req, res) => {
    ...
    //   /design-system.json    returns parsed DESIGN.md and .impeccable/design.json sidecar
    //                          when present. Panel merges them:
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      const jsonPath = resolveDesignSidecarPath(process.cwd(), contextDir) || getDesignSidecarPath(process.cwd());
  ...
  req.on('end', () => {
    ...
    if (state.sessionStore && msg.id) {
      try {
        ...
        // Keep the reply path best-effort; browser still needs the SSE.
      }
    }
    flushPendingPolls();
