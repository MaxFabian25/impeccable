...
function readServerInfo() {
  ...
  if (!record) {
    console.error('No running live server found. Start one with live.mjs.');
    process.exit(1);
  }
  return record.info;
}
...
Options:
  ...
  // Reply mode: node live-poll.mjs --reply <id> <status> [--file path] [message]
  ...
  if (replyIdx !== -1) {
    ...
    if (!id) {
      console.error('Usage: node live-poll.mjs --reply <id> <status> [--file path] [message]');
      process.exit(1);
    }
    ...
    try {
      ...
      if (err.cause?.code === 'ECONNREFUSED') {
        console.error('Live server not running. Start one with live.mjs.');
      } else {
  ...
  try {
    while (true) {
      ...
      if (res.status === 401) {
        ...
        console.error('Try restarting with live-server.mjs stop, then live.mjs.');
        process.exit(1);
      }
