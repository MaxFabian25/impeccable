...
4. **Pull in any prior critique** (optional signal): If $critique has been run on the same target, its priority issues are useful input. Resolve the target to a file path or URL, then:
   ```bash
   slug=$(node skills/impeccable/scripts/critique-storage.mjs slug "<resolved>")
   node skills/impeccable/scripts/critique-storage.mjs latest "$slug"
   ```
