...
If the project is empty or pre-implementation, offer `{{command_prefix}}impeccable document` in seed mode. Seed mode asks a few visual direction questions and writes a starter DESIGN.md without a `.impeccable/design.json` sidecar. Tell the user to refresh it after the first real interface exists.
...
If this skill is invoked with the argument "live" (e.g., `{{command_prefix}}impeccable live`), follow the [live variant flow](reference/live.md). This mode starts the local helper, injects the browser picker into the configured page file, then uses `live-poll.mjs` as the foreground event loop while Codex writes and accepts source-backed variants.
