...
async function loadDesignSystemPayload(projectRoot = process.cwd()) {
  ...
  if (mdStat) {
    try {
      ...
      payload.parsed = parseDesignMd(fs.readFileSync(mdPath, 'utf-8'));
    } catch (err) {
      payload.parseError = err.message;
    }
  }

  if (jsonStat) {
    try {
      payload.sidecar = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));
    } catch (err) {
      payload.sidecarError = 'Failed to parse DESIGN.json: ' + err.message;
    }
  }
...
function normalizeLiveNumber(value, fieldName) {
  const number = Number(value);
  if (!Number.isFinite(number)) throw new Error(`${fieldName} must be a finite number`);
  return Math.round(number * 10) / 10;
}

function normalizeLiveComments(comments) {
  if (comments == null) return [];
  if (!Array.isArray(comments)) throw new Error('comments must be an array');
  return comments.map((comment, index) => {
    if (!comment || typeof comment !== 'object') throw new Error(`comments[${index}] must be an object`);
    return {
      x: normalizeLiveNumber(comment.x, `comments[${index}].x`),
      y: normalizeLiveNumber(comment.y, `comments[${index}].y`),
      text: String(comment.text || '').slice(0, 1000),
    };
  });
}

function normalizeLiveStrokes(strokes) {
  ...
  return strokes.map((stroke, strokeIndex) => {
    if (!stroke || typeof stroke !== 'object' || !Array.isArray(stroke.points)) {
      throw new Error(`strokes[${strokeIndex}].points must be an array`);
    }
    return {
      points: stroke.points.map((point, pointIndex) => {
        if (!Array.isArray(point) || point.length < 2) {
          throw new Error(`strokes[${strokeIndex}].points[${pointIndex}] must be [x, y]`);
        }
        return [
          normalizeLiveNumber(point[0], `strokes[${strokeIndex}].points[${pointIndex}][0]`),
          normalizeLiveNumber(point[1], `strokes[${strokeIndex}].points[${pointIndex}][1]`),
        ];
      }),
    };
  });
...
function normalizeLiveEventPayload(payload) {
  if (!payload || typeof payload !== 'object') throw new Error('event payload must be an object');
  if (!isSafeLiveId(payload.id)) throw new Error('id must be 1-96 chars of letters, numbers, underscore, or dash');
  const type = payload.type || 'annotation';
  if (typeof type !== 'string' || !/^[a-zA-Z0-9_-]{1,40}$/.test(type)) throw new Error('type is invalid');
  const element = payload.element && typeof payload.element === 'object' ? payload.element : {};
  return {
    type,
    id: payload.id,
    pageUrl: typeof payload.pageUrl === 'string' ? payload.pageUrl.slice(0, 4000) : '',
    action: typeof payload.action === 'string' ? payload.action.slice(0, 80) : undefined,
    count: Number.isFinite(Number(payload.count)) ? Number(payload.count) : undefined,
    screenshotPath: typeof payload.screenshotPath === 'string' ? payload.screenshotPath : undefined,
    comments: normalizeLiveComments(payload.comments),
    strokes: normalizeLiveStrokes(payload.strokes),
    element: {
      selector: typeof element.selector === 'string' ? element.selector.slice(0, 2000) : '',
      tagName: typeof element.tagName === 'string' ? element.tagName.slice(0, 80) : '',
      outerHTML: typeof element.outerHTML === 'string' ? element.outerHTML.slice(0, 20000) : '',
      boundingRect: element.boundingRect && typeof element.boundingRect === 'object' ? element.boundingRect : null,
    },
    receivedAt: new Date().toISOString(),
  };
}

function resolveModernScreenshotBrowserPath() {
  const require = createRequire(import.meta.url);
  return require.resolve('modern-screenshot/dist/index.js');
}
...
The server provides:
  ...
  /modern-screenshot.js  Browser capture library used by annotation mode
  /annotation   POST PNG captures from annotation mode
  /events       POST structured live annotation events
  /poll         Long-poll the next live annotation event
  ...
  const modernScreenshotPath = resolveModernScreenshotBrowserPath();

  let browserScript;
  let modernScreenshotScript;
  try {
    browserScript = fs.readFileSync(scriptPath, 'utf-8');
    modernScreenshotScript = fs.readFileSync(modernScreenshotPath, 'utf-8');
  } catch {
    process.stderr.write('Error: Browser script or modern-screenshot dependency not found. Run `npx bun install` and `npm run build:browser` first.\n');
    process.exit(1);
  }
  ...
  const liveEvents = [];
  const livePollers = [];
  const liveSessionDir = path.join(process.cwd(), '.impeccable-live', 'annotations', `session-${Date.now().toString(36)}`);

  const enqueueLiveEvent = (event) => {
    const poller = livePollers.shift();
    if (poller) {
      poller(event);
      return;
    }
  ...
  const validateLiveToken = (requestUrl, bodyToken = '') => {
    const token = bodyToken || requestUrl.searchParams.get('token') || '';
    return token === liveToken;
  };

  const shutdown = () => {
    while (livePollers.length) {
      livePollers.shift()({ type: 'shutdown' });
    }
  ...
  const server = http.default.createServer(async (req, res) => {
    ...
    if (requestPath === '/detect.js' || requestPath === '/') {
      ...
      res.end(`window.__IMPECCABLE_LIVE_TOKEN__=${JSON.stringify(liveToken)};\n${browserScript}`);
    } else if (requestPath === '/modern-screenshot.js') {
      res.writeHead(200, {
        'Content-Type': 'application/javascript',
        'Cache-Control': 'public, max-age=31536000, immutable',
      });
      res.end(modernScreenshotScript);
      ...
      res.end(JSON.stringify({
        ...
        hasAnnotationQueue: true,
      }));
    } else if (requestPath === '/annotation') {
      if (req.method !== 'POST') {
        writeJson(res, 405, { error: 'POST required' });
        return;
      }
      if (!validateLiveToken(requestUrl)) {
        writeJson(res, 401, { error: 'invalid token' });
        return;
      }
      const eventId = requestUrl.searchParams.get('eventId') || '';
      if (!isSafeLiveId(eventId)) {
        writeJson(res, 400, { error: 'invalid eventId' });
        return;
      }
      if (!String(req.headers['content-type'] || '').toLowerCase().startsWith('image/png')) {
        writeJson(res, 415, { error: 'content-type must be image/png' });
        return;
      }
      try {
        const body = await readRequestBody(req);
        if (!isPngBuffer(body)) {
          writeJson(res, 400, { error: 'body must be a PNG image' });
          return;
        }
        fs.mkdirSync(liveSessionDir, { recursive: true });
        const targetPath = path.join(liveSessionDir, `${eventId}.png`);
      ...
      try {
        ...
        if (!validateLiveToken(requestUrl, payload.token)) {
          writeJson(res, 401, { error: 'invalid token' });
          return;
        }
        const event = normalizeLiveEventPayload(payload);
        enqueueLiveEvent(event);
        writeJson(res, 200, { ok: true, id: event.id });
      } catch (err) {
        writeJson(res, 400, { error: err.message });
      }
      ...
      if (liveEvents.length > 0) {
        writeJson(res, 200, liveEvents.shift());
        return;
      }
      const timeoutMs = Math.max(0, Math.min(30000, Number(requestUrl.searchParams.get('timeout') || 0)));
      if (timeoutMs === 0) {
        writeJson(res, 200, { type: 'timeout' });
        return;
      }
      let done = false;
      ...
      livePollers.push(finish);
      req.on('close', () => {
        if (done) return;
        done = true;
        clearTimeout(timer);
        const index = livePollers.indexOf(finish);
        if (index !== -1) livePollers.splice(index, 1);
      });
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    fs.writeFileSync(LIVE_PID_FILE, JSON.stringify({ pid: process.pid, port, sessionDir: liveSessionDir }));
    ...
    console.log(`Annotations are saved under: ${liveSessionDir}\n`);
    console.log(`Poll annotation events:`);
    console.log(`  curl '${url}/poll?token=${liveToken}&timeout=30000'\n`);
