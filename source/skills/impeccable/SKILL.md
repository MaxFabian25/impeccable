...
2. **Load PRODUCT.md + DESIGN.md (fast)**: Run `node {{scripts_path}}/load-context.mjs` from the project root. If `migrated` is true, legacy `.impeccable.md` was renamed to `PRODUCT.md`; mention that once. If `hasProduct` is true, proceed. If `hasDesign` is false and the task needs visual consistency, briefly suggest `{{command_prefix}}impeccable document` after the current work.
3. **Run impeccable teach (REQUIRED)**: If `hasProduct` is false, you MUST run {{command_prefix}}impeccable teach NOW before doing anything else. Do NOT skip this step. Do NOT attempt to infer strategic context from the codebase instead.
...
If the project is empty or pre-implementation, skip DESIGN.md and tell the user to run `{{command_prefix}}impeccable document` after the first real interface exists.
