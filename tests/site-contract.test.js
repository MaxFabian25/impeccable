...
describe('codex-only site contract', () => {
  ...
  test('cheatsheet renders dollar-prefixed commands from the shared data module', () => {
    const cheatsheet = readFileSync(join(REPO_ROOT, 'site', 'pages', 'cheatsheet.astro'), 'utf8');
  ...
  test('slop is canonical and old public pages redirect', () => {
    ...
    const astroConfig = readFileSync(join(REPO_ROOT, 'astro.config.mjs'), 'utf8');
    ...
    expect(buildScript).not.toContain('Bun.build');
    expect(packageJson).toContain('"build:site": "npx astro build"');
    expect(packageJson).toContain('"dev": "npx astro dev"');
    expect(astroConfig).toContain("srcDir: './site'");
  });
  ...
  test('visualize-first site flow stays Codex-only and has source assets', () => {
    ...
    expect(designingPage).toContain('Start with words, pictures, then code.');
    expect(designingPage).toContain('OpenAI image generation');

    for (const source of [homepage, designingPage]) {
