...
import { loadContext, resolveContextDir } from '../skills/impeccable/scripts/load-context.mjs';
...
describe('loadContext', () => {
  ...
  it('keeps local register and seed-design metadata', () => {
    write('PRODUCT.md', '# Product\n\n## Register\n- product\n');
    write('DESIGN.md', '<!-- SEED -->\n# Design\n');

    const ctx = loadContext(scratch);

    assert.equal(ctx.register, 'product');
    assert.equal(ctx.designIsSeed, true);
  });

  it('migrates legacy .impeccable.md to PRODUCT.md at root', () => {
